...
/**
 * Send reminder to candidates who haven't completed their interviews
 * @param {number} daysOld - How many days old the interview should be to send a reminder
 * @returns {Promise<Object>} - Result of the reminder operation
 */
const sendInterviewReminders = async (daysOld = 3) => {
  try {
    ...
    // Find all HRs with pending interviews older than the cutoff date
    ...
    for (const hr of hrs) {
      for (const interview of hr.interviews) {
        // Skip interviews that are newer than the cutoff date
